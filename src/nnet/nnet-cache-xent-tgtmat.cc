// nnet/nnet-cache-xent-tgtmat.cc
...
void CacheXentTgtMat::Init(int32 cachesize, int32 bunchsize) {
...
void CacheXentTgtMat::AddData(const CuMatrix<BaseFloat> &features, const std::vector<int32> &aligns, const CuMatrix<BaseFloat> &targets) {
  if (state_ == FULL) {
    KALDI_ERR << "Cannot add data, cache already full";
  }

  assert(features.NumRows() == targets.NumRows() && features.NumRows() == aligns.size());
  ...
  if (state_ == EMPTY) { 
    ...
    if (leftover > 0) {
      ...
      std::copy(aligns_leftover_.begin(), aligns_leftover_.begin()+leftover, aligns_.begin());
      targets_.CopyRowsFromMat(leftover, targets_leftover_, 0, 0);
      
      features_leftover_.Destroy();
      aligns_leftover_.resize(0);
  ...
  if (leftover > 0) {
    ...
    assert((aligns.end() - (aligns.begin()+fill_rows)) == leftover);
    aligns_leftover_.resize(leftover);
    std::copy(aligns.begin()+fill_rows, aligns.end(), aligns_leftover_.begin());
...
void CacheXentTgtMat::Randomize() {
  ...
  aligns_random_.resize(cachesize_);
  ...
  // randomize the aligns
  for(int32 i=0; i<filling_pos_; ++i){
    aligns_random_[i]=aligns_[randmask_[i]];
  }
...
void CacheXentTgtMat::GetBunch(CuMatrix<BaseFloat> *features, std::vector<int32> *aligns, CuMatrix<BaseFloat> *targets) {
  if (state_ == EMPTY) {
  ...
  if (randomized_) {
    ...
    std::copy(aligns_.begin()+emptying_pos_, aligns_.begin()+emptying_pos_+bunchsize_, aligns->begin());
