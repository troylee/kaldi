...
void Nnet::Backpropagate(const CuMatrix<BaseFloat> &in_err,
                         CuMatrix<BaseFloat> *out_err) {
  if (LayerCount() == 0) {
    KALDI_ERR<< "Cannot backpropagate on empty network";}
...
void Nnet::Feedforward(const CuMatrix<BaseFloat> &in,
...
void Nnet::SetLearnRate(BaseFloat lrate, const char *lrate_factors) {
  ...
  if (lrate_factor_vec.size() > 0
      && updatable != (int32) lrate_factor_vec.size()) {
  ...
  for (int32 i = 0; i < LayerCount(); i++) {
    if (nnet_[i]->IsUpdatable()) {
      ...
      if (lrate_factor_vec.size() > 0)
        lrate_scaled *= lrate_factor_vec[updatable++];
