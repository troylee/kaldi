...
void Relu(const CuMatrix<Real>& X, CuMatrix<Real>* Y) {
#if HAVE_CUDA==1
  if(CuDevice::Instantiate().Enabled()){
    ...
    cuda_relu(dimGrid, dimBlock, Y->Data(), X.Data(), X.Dim());
...
void DiffRelu(const CuMatrix<Real>& Ein, const CuMatrix<Real>& Y, CuMatrix<Real>* Eout) {
  ...
  if(CuDevice::Instantiate().Enabled()) {
    ...
    cuda_diff_relu(dimGrid, dimBlock, Eout->Data(), Ein.Data(), Y.Data(), Eout->Dim());
    cuSafeCall(cudaGetLastError());
...
template<typename Real>
void SoftRelu(const CuMatrix<Real>& X, CuMatrix<Real>* Y) {
#if HAVE_CUDA==1
  if(CuDevice::Instantiate().Enabled()){
    Timer tim;
    ...
    cuda_softrelu(dimGrid, dimBlock, Y->Data(), X.Data(), X.Dim());
    cuSafeCall(cudaGetLastError());
...
#endif
  {
    ...
    for(MatrixIndexT r=0; r<x.NumRows(); r++) {
      for(MatrixIndexT c=0; c<x.NumCols(); c++) {
        y(r, c) = (x(r,c) > 4.0) ? x(r, c) : log(1+exp(x(r,c)));
      }
    }
  }
}
...
void DiffSoftRelu(const CuMatrix<Real>& Ein, const CuMatrix<Real>& X, CuMatrix<Real>* Eout) {
  ...
  if(CuDevice::Instantiate().Enabled()) {
    ...
    cuda_diff_softrelu(dimGrid, dimBlock, Eout->Data(), Ein.Data(), X.Data(), Eout->Dim());
    cuSafeCall(cudaGetLastError());
