...
template<typename Real>
__global__
static void _part_set_const(Real* mat, Real value, MatrixDim d, int32_cuda ro, int32_cuda r, int32_cuda co, int32_cuda c) {
  int32_cuda i = blockIdx.x * blockDim.x + threadIdx.x;
  int32_cuda j = blockIdx.y * blockDim.y + threadIdx.y;
  int32_cuda index = i + j*d.stride;
  if ( i >= co && i < co+c && j >= ro  && j < ro+r )
    mat[index] = value;
}
...
void cudaF_part_set_const(dim3 Gr, dim3 Bl, float* mat, float value, MatrixDim d, int32_cuda ro, int32_cuda r, int32_cuda co, int32_cuda c) {
  _part_set_const<<<Gr,Bl>>>(mat,value,d,ro,r,co,c);
}
