// nnetbin/lin-init-ae.cc

/*
 * Initialize a LIN NN with SI Autoencoder NN.
 *
 */
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Append a LIN to the existing Autoencoder nnet.\n"
            "Usage:  lin-init-ae [options] <nnet-in> <nnet-out>\n"
            "e.g.:\n"
            " lin-init-ae --binary=false nnet.mdl nnet_lin.mdl\n";
    ...
    int32 in_dim = nnet.InputDim();
    {
      ...
      // for AE, we need to put LinBL both at front and at the end
      lin.Write(ko.Stream(), binary_write);
    }
