  ...
  try {
    const char *usage =
        "Perform one iteration of Neural Network training by stochastic gradient descent.\n"
            "Usage:  gaussbl-train [options] <model-in> <feature-rspecifier> "
            "<noise-rspecifier> <alignments-rspecifier> [<model-out|output-wspecifier>]\n"
            "e.g.: \n"
            " gaussbl-train nnet.init scp:train.scp ark:noise.ark ark:train.ali nnet.iter1\n";
    ...
    if (nnet.Layer(0)->GetType() != Component::kGaussBL) {
      KALDI_ERR<< "The first layer is not <gaussbl> layer!";
    }
    GaussBL *posnegbl_layer = static_cast<GaussBL*> (nnet.Layer(0));
    if (!cross_validate) {
    ...
    for (; !feature_reader.Done(); /*feature_reader.Next()*/) {
      ...
      if (!alignments_reader.HasKey(key)) {
        ...
        posnegbl_layer->SetNoise(compensate_var, mu_h, mu_z, var_z);
