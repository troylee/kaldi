...
parser.add_option('--dropInput', dest='dropInput', help='whether to drop inputs', default=False)
parser.add_option('--inputDropRatio', dest='inputDropRatio', help='drop ratio for inputs', default=0.2)
parser.add_option('--hiddenDropRatio', dest='hiddenDropRatio', help='drop ratio for hidden layers', default=0.5)
...
if dropInput:
    print '<dropout>', dimL[0], dimL[0]
    print inputDropRatio
...
for layer in range(len(dimL)-1):
    ...
    if(int(options.linBNdim) != dimL[layer+1]):
        if(layer == len(dimL)-2):
            ...
            print '<relu>', dimL[layer+1], dimL[layer+1]
            print '<dropout>', dimL[layer+1], dimL[layer+1]
            print hiddenDropRatio
