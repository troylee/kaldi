...
if [ $# != 7 ]; then
   echo "Usage: $0 <rbm-in> <mlp-init> <feats-tr> <feats-cv> <labels-tr> <labels-cv> <exp-dir>"
...
$train_tool --cross-validate=true \
 ...
 ${hidbias_cv:+ --hidbias=$hidbias_cv} \
 "$feats_cv" "$labels_cv" $rbm_mdl $mlp_best \
 2>> $log || exit 1;
...
for iter in $(seq -w $max_iters); do
  ...
  $train_tool \
   ...
   "$feats_tr" "$labels_tr" $rbm_mdl $mlp_best $mlp_next \
   2>> $log || exit 1; 
  ...
  $train_tool --cross-validate=true \
   ...
   ${feature_transform:+ --feature-transform=$feature_transform} \
   ${hidbias_cv:+ --hidbias=$hidbias_cv} \
   "$feats_cv" "$labels_cv" $rbm_mdl $mlp_next \
   2>>$log || exit 1;
