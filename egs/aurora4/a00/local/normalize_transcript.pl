...
while(<STDIN>) {
    ...
    foreach $w (split (" ",$trans)) {
		...
		$w =~ s:^EXISITING$:EXISTING:; # Spelling error in the transcript
		$w =~ s:^OUTSIDING$:OUTSIDE ING:; # wrong word for dev transcripts, should be outstanding in fact
        if($w =~ m:^\[\<\w+\]$:  || # E.g. [<door_slam], this means a door slammed in the preceding word. Delete.
           ...
           $w eq ".") {      # "." is used to indicate a pause.  Silence is optional anyway so not much 
			...
			# mispronunciations, still keep the word to add some variations
			print " $1";
		} elsif($w =~ m:^(.*)\!(.+)$:) {
			# emphatic stree, also kept
			if($2 eq "EXCLAMATION-POINT") {
				print " $w";
			} else {
				$w = $1.$2;
				if($w =~ m:^(.+)\:(.*)$:){ # one exception case
					print " $1$2";
				} else {
					print " $w";
				}
			...
			# abnormal lengthening, also kept
			print " $1$2";
        } elsif($w eq "--DASH") {
